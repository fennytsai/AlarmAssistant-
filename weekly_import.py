...
print("匯入範圍:", start_date, "~", end_date)
# =========================
# 清除過期資料
# =========================
remove_old_appointments(start_date.strftime("%Y-%m-%d"))
# =========================
# 讀取目前 Sheet
# 只讀一次
# =========================
existing = get_existing_appointments()
print("目前已有資料:", len(existing))
# =========================
# 準備新增資料
# =========================
new_rows = []
...
while current <= end_date:
    ...
    if current.weekday() == 5:
        ...
        for doctor in DOCTORS:
            ...
            if key in existing:
                print("已存在:", current, doctor["name"])
            else:
                print("準備新增:", current, doctor["name"])
                new_rows.append({
...
# =========================
if new_rows:
    add_appointments_batch(new_rows)
    print("新增完成:", len(new_rows), "筆")
else:
    print("沒有需要新增資料")
